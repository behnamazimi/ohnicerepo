...
import type { ApiResponse, FilterParams, RateLimit } from '../types';
...
export function useRepos(params: FilterParams) {
  ...
  const fetchRepos = useCallback(async (skipCache = false) => {
    setLoading(true);
    setError(null);
  ...
  useDebounce(
    () => fetchRepos(),
    400,
    [params.days, params.stars, params.language, params.dateType]
  );
